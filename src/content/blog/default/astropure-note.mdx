...
### rehype-tabs 报错的解决
```ts title="src\packages\astro-pure\plugins\rehype-tabs.ts"
// ...
interface Panel { // [!code --]
export interface Panel { // [!code ++]
  panelId: string
  tabId: string
  label: string
}
// ...
export const processPanels = (html: string) => { // [!code --]
export const processPanels = (html: string): { panels: Panel[]; html: string } => { // [!code ++]
  const file = tabsProcessor.processSync({ value: html })
  return {
    panels: file.data.panels, // [!code --]
    panels: file.data.panels as Panel[], // [!code ++]
    html: file.toString()
  }
}
```
...
const typographyConfig = {
    ...
    pre: { // [!code ++]
      color: fg // [!code ++]
   	...
   	'.dark blockquote': { // [!code ++]
   	  color: 'hsl(var(--muted-foreground))' // [!code ++]
   	}, // [!code ++]
   	'.dark :not(pre) > code': { // [!code ++]
   	  color: 'hsl(var(--foreground))' // [!code ++]
   	}, // [!code ++]
		// Blockquote
    blockquote: {
    	//...
