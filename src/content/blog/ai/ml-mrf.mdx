...
\end{aligned}
$$
这样变形的目的，是为了凑出对数几率函数 $\mathrm{logit}(p)=\log \dfrac{p}{1-p}$，因为它是 sigmoid 的反函数，而这里 $p=\sigma(w^\mathrm{T}x)$，故 $\log \dfrac{p}{1-p}$ 就等于 $w^\mathrm{T}x$<span class="text-foreground/40">（因此 logistic 回归也称为对数几率回归）</span>。于是代入后继续化简得
$$
\begin{aligned}
